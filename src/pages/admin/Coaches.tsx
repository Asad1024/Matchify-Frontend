...
export default function Coaches() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['/api/admin/coaches', search, page],
    queryFn: async () => {
      const url = buildApiUrl(`/api/admin/coaches?search=${encodeURIComponent(search)}&page=${page}&limit=50`);
      ...
      <div className="p-4 sm:p-6 space-y-4">
        <div className="flex flex-wrap justify-between items-center gap-2">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Manage all coaches</p>
          </div>
          <div className="relative">
            <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search coaches..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="pl-8 w-48" />
          </div>
        </div>
          ...
          <Card>
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {coaches.length === 0 ? (
                    <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No coaches found</TableCell></TableRow>
                  ) : coaches.map((c: any) => (
                    <TableRow key={c.id}>
